...
Options get_options(int argc, char *argv[]) {
  ...
  while ((k = getopt(argc, argv, "c:d:k:p:r:i:o:t:v:z")) != -1) {
    ...
    default:
      fprintf(stderr,
              "Usage: ./feynqft -i input_bitstring -o output_bitstring\n");
      exit(1);
    }
  }
  return opts;
...
bool lookup_intenal_wire(int history, int wire, int internal_index) {
  return (history >> wire * (2 * NUM_QUBITS - wire + 1) / 2 + internal_index) &
         1;
}
...
int main(int argc, char *argv[]) {
  ...
  //         cout << "(wire: " << gq.wire << ", index: " << gq.internal_index_in
  //         << ", control: " << gq.is_control << ") ";
  //     }
  //     cout << "\n";
  // }
  ...
  // OpenMP
  for (u_int64_t history = 0; history < u_int64_t(1) << num_internal_wires;
       history++) {
    ...
    for (const Gate &gate : Circuit::gates) {
      ...
      case CPHASE:
        ...
        if (gate.qubits[0].at_input && gate.qubits[0].at_output) {
          if (input_bits[gate.qubits[0].wire] !=
              output_bits[gate.qubits[0].wire]) {
            contribution *= 0.0;
            break;
          } else {
      ...
      // gate_type_to_string(gate.type).c_str(), contribution.real(),
      // contribution.imag());
    }
    // printf("Contribution from history %lu: %f + i%f\n", history,
    // contribution.real(), contribution.imag());
    total_amplitude += contribution;
  }

  printf("Total amplitude: %f + i%f\n", total_amplitude.real(),
         total_amplitude.imag());
