// Lightweight executable to calculate amplitude for certain input and output
// bitstring
...
Options get_options(int argc, char *argv[]) {
  ...
  const char *helpstr =
      "Usage: ./bitstr(_mpi/omp).x -c circuit_file -i input_bitstring -o "
      "output_bitstring -p num_chunk1 -r num_chunk2 (-B)\n";
  ...
  while ((k = getopt(argc, argv, "c:i:o:p:r:f:B")) != -1) {
    ...
    case 'c':
      ...
      break;
    case 'i': // Not necessary. We could leave to the user to initialize in
              // circuit_file and always start at state |0>.
...
int main(int argc, char *argv[]) {
  ...
  // TODO: Autotuning: Build circuit many times with different cp-params, save
  // the best params and build once more for those.
  ...
  for (int i = 0; i < NUM_CHUNKS; i++) {
    printf("Chunk %d with %zu gates and %d artificial sources:\n", i,
           Circuit::chunks.at(i).gates.size(),
           Circuit::chunks.at(i).num_artificial);
  ...
  //        Circuit::chunks.at(i).internal_wires.size()); for (const
  //        shared_ptr<InternalWire>& iw : Circuit::chunks.at(i).internal_wires)
  //        {
