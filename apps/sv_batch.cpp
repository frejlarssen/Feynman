 ...
 * Fixed batch size for each MPI process --> batch_size = tot_states /
 * MPI_processes
 */
...
Options get_options(int argc, char *argv[]) {
  ...
  const char *helpstr =
      "Usage: ./sv(_mpi/omp).x -c circuit_file -i input_sv -o output_sv -p "
      "num_chunk1 -r num_chunk2 -f fraction_of_histories -t threshold -v "
      "verbosity (-D [Dense])\n";
...
int main(int argc, char *argv[]) {
  ...
  for (std::size_t output_int = my_worker; output_int < total_output_bitstrings;
    ...
    while (getline(in_file, in_line)) {
      ...
      if (opts.dense) {
        ...
        input_bits =
            bit_array_from_int(string_to_int128(basis_state_str), Circuit::n);
  ...
  MPI_File fh;
  if (world_rank == 0)
    MPI_File_delete(opts.output_statevector_file.c_str(), MPI_INFO_NULL);
  MPI_Barrier(MPI_COMM_WORLD);
  int rc = MPI_File_open(MPI_COMM_WORLD, opts.output_statevector_file.c_str(),
  ...
  if (world_rank == 0)
    my_offset = 0;
  MPI_Status st;
  MPI_File_write_at_all(fh, my_offset, (void *)local_buf.data(), (int)my_bytes,
                        MPI_BYTE, &st);
  MPI_File_close(&fh);
