...
struct GateQubit {
    ...
    const int internal_index_in; // Might not be necessary. Should maybe be out instead.
    ...
    GateQubit(int w, int idx, bool ctrl, bool in=false, bool out=false)
        : wire(w), internal_index_in(idx), global_index(w * (2*NUM_QUBITS - w + 1 ) / 2 + idx), is_control(ctrl), at_input(in), at_output(out) {}
};
    ...
    static vector<Gate> build_qft() {
        ...
        for (int i = 0; i < n / 2; i++) {
            g.emplace_back(SWAP, vector<GateQubit>{
                GateQubit(n - i - 1, 0, false, false, true),
